...
Game *New_Game()
{
    ...
    game->title = "Final Project 113034036 113034006";
    game->display = NULL;
    game->game_init(game);
    return game;
}
void execute(Game *self)//設定運行時"時間"、"鍵盤"、"滑鼠"等控制
{
    // main game loop
    bool run = true;
    while (run)
    {
        ...
        switch (event.type)//依event型別分類
        {
        case ALLEGRO_EVENT_TIMER://時間參數
        {
...
bool game_update(Game *self)
{
    ...
    if(HoverOnItem) {al_set_system_mouse_cursor(self->display, ALLEGRO_SYSTEM_MOUSE_CURSOR_LINK);HoverOnItem=false;}
    else al_set_system_mouse_cursor(self->display, ALLEGRO_SYSTEM_MOUSE_CURSOR_DEFAULT);
    return true;
}
